...
QFuture<QByteArray> DataOffer::DataOffer::receive(const std::string &mime) {
  ...
  QTimer::singleShot(0, [this, mime, promise = std::move(promise)]() mutable {
    int pipefd[2];
    if (pipe(pipefd) != 0) { throw std::runtime_error(std::string("Failed to pipe(): ") + strerror(errno)); }

    qDebug() << "write fd" << pipefd[1] << "read fd" << pipefd[0];
    ...
    // wl_display_dispatch(qApp->nativeInterface<QNativeInterface::QWaylandApplication>()->display());
    close(pipefd[1]);
    ...
    QObject::connect(notifier, &QSocketNotifier::activated,
                     [notifier, promise = std::move(promise)]() mutable {
                       int rc = 0;
